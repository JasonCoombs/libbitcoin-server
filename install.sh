...
build_all()
{
if [ ! -d "$BUILD_DIR/libbitcoin-server" ]; then
    ...
    build_from_github JasonCoombs libbitcoin-node master $PARALLEL ${BITCOIN_NODE_OPTIONS[@]} "$@"
    build_from_github JasonCoombs libbitcoin-protocol master $PARALLEL ${BITCOIN_PROTOCOL_OPTIONS[@]} "$@"
    build_from_travis JasonCoombs libbitcoin-server master $PARALLEL ${BITCOIN_SERVER_OPTIONS[@]} "$@"
else
