...
options_metadata parser::load_environment(config::configuration *pconf)
{
    ...
    (
        ...
        BS_CONFIG_VARIABLE,
        value<path>(&servconf->configfile)->composing()
...
*/
    ...
    (
        "network.blacklist",
        value<config::authority::list>(&servconf->network->blacklists),
        "IP address to disallow as a peer, multiple entries allowed."
    )
    (
        "network.peer",
        value<config::endpoint::list>(&servconf->network->peers),
        "A persistent peer node, multiple entries allowed."
    )
    (
        "network.seed",
        value<config::endpoint::list>(&servconf->network->seeds),
...
/*    (
 ...
 TODO: re-enable configurable database directory, must adjust for testnet */
    ...
    (
        ...
        value<bool>(&servconf->network->relay_transactions),
        "Request that peers relay transactions, defaults to false."
    )
    (
        "node.refresh_transactions",
        value<bool>(&servconf->node->refresh_transactions),
    ...
    (
        ...
        value<bool>(&servconf->database->index_addresses),
    ...
    (
        ...
        value<config::checkpoint>(&servconf->bitcoin->bip9_bit0_active_checkpoint),
    ...
    (
        ...
        value<config::checkpoint>(&servconf->bitcoin->bip9_bit1_active_checkpoint),
    ...
    (
        ...
        value<uint64_t>(&servconf->bitcoin->initial_block_subsidy_bitcoin_),
        "The initial block subsidy in bitcoin, defaults to 50."
    )
    (
        "bitcoin.subsidy_interval",
        value<uint64_t>(&servconf->bitcoin->subsidy_interval_),
